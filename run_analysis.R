...
## assuming there is a 'data' directory in you working directory which contains the needed data
...
# rbind data with reference to partition
df <- rbind(X_train[,partition:=factor(rep(1, .N), levels = c(1, 2), labels = c("train", "test"))], 
           X_test[,partition:=factor(rep(2, .N), levels = c(1, 2), labels = c("train", "test"))]) 
colnames(df) <- c(features, "partition")
...
table(df$activity) # check labels
# add subject
